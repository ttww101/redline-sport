...
#import "QiciModel.h"
@interface SelectedDataView()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
@property (nonatomic, strong) UITableView *tableView;

@end
@implementation SelectedDataView
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(SelecterMatchView:selectedAtIndex:WithSelectedName:)]) {
        
        if (_arrData.count >0) {
            
            QiciModel *qici = [_arrData objectAtIndex:indexPath.row];
            
            [_delegate SelecterMatchView:self selectedAtIndex:indexPath.row WithSelectedName:qici.name];
        }
    }
}
@end
