...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[SaiTableViewCell class] forCellReuseIdentifier:cellGuanzhuViewController];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SaiTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellGuanzhuViewController];
    if (!cell) {
        cell = [[SaiTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellGuanzhuViewController];
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_arrData.count > 0) {
        ...
        LiveScoreModel *model = [_arrData objectAtIndex:indexPath.row];
...
- (void)getAttention
{
    ...
    [[DCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_bifen_focus] ArrayFile:nil Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            _arrData = [[NSMutableArray alloc] initWithArray:[LiveScoreModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"matchs"]]];
            [self.tableView reloadData];
            
            NSMutableArray *arrMid = [NSMutableArray array];
            for (int i = 0; i<_arrData.count; i++) {
                LiveScoreModel *liveM = [_arrData objectAtIndex:i];
