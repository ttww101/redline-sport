...
@interface SelectedJincaiVC ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,AllSelectedViewDelegate>
...
@implementation SelectedJincaiVC
...
//            //将上次筛选的记录到本地
//            NSString *documentPath = [Methods getDocumentsPath];
//            NSString *arrSaveBifenAllSelected = [documentPath stringByAppendingPathComponent:arrSaveBifenJingcaiSelectedPath];
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        [self.collectionView registerClass:[SelectedCCell class] forCellWithReuseIdentifier:cellCollectioneSlectedJincai];
...
{
    SelectedCCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellCollectioneSlectedJincai forIndexPath:indexPath];
    if (!cell) {
        NSLog(@"无法创建cell");
    }
...
//    NSMutableArray *arr = [_arrItemData objectAtIndex:indexPath.section];
    BIfenSelectedSaishiModel *model = [_arrData objectAtIndex:indexPath.row];
...
//    UILabel *labTitle = [[UILabel alloc] initWithFrame:CGRectMake(20, 0, 100, 30)];
//    labTitle.textColor = grayColor34;
//    labTitle.font = font13;
//    labTitle.text = _arrSectionData[indexPath.section];
//    NSLog(@"%@",_arrSectionData[indexPath.section]);
