...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        [_tableView registerClass:[PeiLvDetailCell class] forCellReuseIdentifier:peilvDetailCellID];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PeiLvDetailCell *peilvCell = [tableView cellForRowAtIndexPath:indexPath];
    if (!peilvCell) {
        peilvCell = [[PeiLvDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:peilvDetailCellID];
    }
    ...
    if (self.PeiLvCDetailType == isDetailBeforeTwo) {
        ...
        if (_peiLvDetailArr.count > 0) {
            PeiLvDetailModel *nowModel = _peiLvDetailArr[indexPath.row];
           
            if (_peiLvDetailArr.count > 1) {
                
                if (indexPath.row!= _peiLvDetailArr.count-1) {
                    
                    PeiLvDetailModel *nextModel = _peiLvDetailArr[indexPath.row +1];
        ...
        if (_peiLvAfterArr.count > 0) {
            ...
            PeiLvDetailModel *nowModel = _peiLvAfterArr[indexPath.row];
            
            if (_peiLvAfterArr.count > 1) {
                
                if (indexPath.row!= _peiLvAfterArr.count-1) {
                    
                    PeiLvDetailModel *nextModel = _peiLvAfterArr[indexPath.row +1];
...
- (void)loadPeiLvDataWithScheid:(NSInteger)scheid OddsType:(oddsType)oddsType isHalf:(isHalfType)isHalf {
   ...
   // [SVProgressHUD showImage:[UIImage imageNamed:@""] status:@"数据加载中..."];
    ...
    [[DCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:lsURl Start:^(id requestOrignal) {
        ...
        _peiLvDetailArr = [[NSMutableArray alloc] initWithArray:[PeiLvDetailModel arrayOfEntitiesFromArray:responseOrignal]];
        if (_peiLvDetailArr.count>0) {
            PeiLvDetailModel*model=_peiLvDetailArr[_peiLvDetailArr.count-1];
            model.BlackType=1;
