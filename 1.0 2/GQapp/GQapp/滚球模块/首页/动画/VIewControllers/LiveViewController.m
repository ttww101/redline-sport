...
@interface LiveViewController () <TYTabPagerControllerDataSource, TYTabPagerControllerDelegate, TYTabPagerBarDelegate>
...
- (void)loadData{
    ...
    [[DCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:@"http://api.live.gunqiu.com:88/radar?action=getDays" Start:^(id requestOrignal) {
...
- (CGFloat)pagerTabBar:(TYTabPagerBar *)pagerTabBar widthForItemAtIndex:(NSInteger)index {
    ...
    CGFloat width = [Methods widthForString:newsPageInfo[@"name"] fontSize:17.f andHeight:0];
    return width;
}
...
- (UIViewController *)tabPagerController:(TYTabPagerController *)tabPagerController controllerForIndex:(NSInteger)index prefetching:(BOOL)prefetching {
    NSDictionary *newsPageInfo = _newsPageInfos[index];
    LiveListViewController *control = [[LiveListViewController alloc]init];
