...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self == [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.basicView addSubview:self.labTZB];
...
- (UILabel *)labTZB{
    if (!_labTZB) {
        _labTZB = [[UILabel alloc] init];
        _labTZB.text = @"74%";
        _labTZB.textColor = color33;
        _labTZB.font = font11;
        _labTZB.textAlignment = NSTextAlignmentCenter;
    }
    return _labTZB;
}
- (UILabel *)labWuCha{
...
-(void)setModel:(TouZhuModel *)model{
    _model = model;
...
- (void)setMas{
    ...
    [self.labTZB mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.labgaiLv mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(self.labTZB.mas_left);
