...
//  GQapp
//
//  Created by 叶忠阳 on 2017/5/10.
//  Copyright © 2017年 GQXX. All rights reserved.
//

#import "TodayHotSpotsCell.h"

@interface TodayHotSpotsCell()
...
@property (nonatomic, strong)UILabel *labBlackNum;//黑色数字
@property (nonatomic, strong)UILabel *labBlackStr;//历史最高
@property (nonatomic, strong)UIView *lineShuView;//竖线
...
@implementation TodayHotSpotsCell
...
-(void)setModel:(MostModel *)model{
    ...
    if (!isNUll( self.labContent.text)) {
        self.labContent.attributedText = [Methods setTextStyleWithString:self.labContent.text WithLineSpace:5.5 WithHeaderIndent:200];
...
//            self.labRedNum.text = model.maxname;
        ...
        case 1:{
            ...
            if (!isNUll(self.labRedNum.text) ) {
                self.labRedNum.attributedText = [Methods withContent:self.labRedNum.text WithColorText:@"%" textColor:redcolor strFont:font17];
                if (model.type == 3) {
    ...
    if (!_yro) {
        _yro = YES;
        [self setMas];
//        [self cellOneIndexPath:self.row];
    }
