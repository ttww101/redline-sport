...
@interface BuyRecordsVC ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
- (void)loadData
{
    ...
    [[DCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_BuyersUrl] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            if (_limitStart == 0) {
                _arrData = [[NSMutableArray alloc] initWithArray:[BuyerModel arrayOfEntitiesFromArray: [responseOrignal objectForKey:@"data"]]];
            }else{
                NSArray *arr = [[NSMutableArray alloc] initWithArray:[BuyerModel arrayOfEntitiesFromArray: [responseOrignal objectForKey:@"data"]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BuyerModel *model = self.arrData[indexPath.row];
...
//    imgPhoto.image = [UIImage imageNamed:@"defaultPic"];
    ...
    labTime.text = [NSString stringWithFormat:@"%@",[Methods getDateByStyle:@"MM-dd HH:mm" withDate:[NSDate dateWithTimeIntervalSince1970:[model.created doubleValue]/1000]]];;
