...
@interface TuijianDetailTableView()<UITableViewDelegate,UITableViewDataSource,TuijianDetailCommentCellDelegate,UIWebViewDelegate>
...
@end
@implementation TuijianDetailTableView
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[TuijianDetailHeaderView class] forCellReuseIdentifier:CellTuijianDetailHeader];
        
        [self registerClass:[TuijianDetailCommentCell class] forCellReuseIdentifier:CellTuijianDetailComment];
...
- (void)setPayUsersModel:(payUserModel *)payUsersModel {
...
- (void)setTuijianModel:(TuijiandatingModel *)tuijianModel {
    
    _tuijianModel = tuijianModel;
}


- (void)setHeaderModel:(TuijiandatingModel *)headerModel
{
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if (_typeTuijianDetailHeader == typeTuijianDetailHeaderCellDanchang) {
            ...
            return [tableView fd_heightForCellWithIdentifier:CellTuijianDetailHeader configuration:^(TuijianDetailHeaderView* cell) {
        ...
        return [tableView fd_heightForCellWithIdentifier:CellTuijianDetailComment cacheByIndexPath:indexPath configuration:^(TuijianDetailCommentCell* cell) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        if (_typeTuijianDetailHeader == typeTuijianDetailHeaderCellDanchang) {
//            TuijianDetailHeaderView *cell = [tableView dequeueReusableCellWithIdentifier:CellTuijianDetailHeader];
//            if (!cell) {
             TuijianDetailHeaderView   *cell = [[TuijianDetailHeaderView alloc] init];
//            }
        ...
        TuijianDetailCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:CellTuijianDetailComment];
        if (!cell) {
            cell = [[TuijianDetailCommentCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellTuijianDetailComment];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.delegate = self;
        cell.type = typeCommentCellTuijian;
...
//        CGFloat paySeeViewHeight = 45;
        ...
        [lab setAttributedText:[Methods withContent:lab.text WithColorText:[NSString stringWithFormat:@"%ld",_headerModel.comment_count] textColor:redcolor strFont:font14]];
...
//    }
    ...
    if ([Methods getUserModel].idId != _headerModel.user_id) {
    ...
    BuyRecordsVC *buyerVC = [[BuyRecordsVC alloc] init];
//    buyerVC.arrData = _buyerArr;
...
- (void)touchBasicViewToHideHudViewWithIdid:(NSInteger)Idid
{
    for (int i = 0; i<_arrCells.count; i++) {
        CommentModel *model = [_arrData objectAtIndex:i];
        if (model.Idid != Idid) {
            TuijianDetailCommentCell *cell = [_arrCells objectAtIndex:i];
            [cell hideHudView];
        }
