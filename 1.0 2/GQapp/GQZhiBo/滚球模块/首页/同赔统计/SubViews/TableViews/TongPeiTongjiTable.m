...
#import "TongbeiTongjiCell.h"
#import "TongPeiTongjiTable.h"
#import "TongPeiDetailVC.h"
@interface TongPeiTongjiTable()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@end
@implementation TongPeiTongjiTable
...
- (instancetype)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[TongbeiTongjiCell class] forCellReuseIdentifier:cellTongPeiTongjiTable];
        self.delegate =self;
...
{
    TongbeiTongjiCell *cell = [tableView dequeueReusableCellWithIdentifier:cellTongPeiTongjiTable];
    if (!cell) {
        cell = [[TongbeiTongjiCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellTongPeiTongjiTable];
    }
...
{
    TongPeiDetailVC *tongPDT = [[TongPeiDetailVC alloc] init];
    TongPeiTJModel *model;
    if (_arrData.count > 0) {
...
{
    NSMutableDictionary *patameter = [NSMutableDictionary dictionaryWithDictionary:[HttpString getCommenParemeter]];
    ...
    [[DCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:patameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_sameOdd_index] Start:^(id requestOrignal) {
...
//            _arrData = [NSArray arrayWithArray:[TongPeiTJModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
