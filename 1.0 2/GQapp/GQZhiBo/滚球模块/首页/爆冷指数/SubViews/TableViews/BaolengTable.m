...
#import "BaolengZhishuCell.h"
#import "BaolengDetailVC.h"
#import "BaolengZishuModel.h"

#import "BaolengTable.h"
@interface BaolengTable()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[BaolengZhishuCell class] forCellReuseIdentifier:cellBaolengZhishuVC];
...
{
    BaolengZhishuCell *cell = [tableView dequeueReusableCellWithIdentifier:cellBaolengZhishuVC];
    if (!cell) {
        cell = [[BaolengZhishuCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellBaolengZhishuVC];
    }
...
{
    BaolengDetailVC *baolengDT = [[BaolengDetailVC alloc] init];
    BaolengZishuModel* model = [_arrData objectAtIndex:indexPath.row];
...
- (void)loadBaolengData
{
    ...
    [[DependetNetMethods sharedInstance] requeSurprisestatisWithType:mtype Start:^(id requestOrignal) {
...
//        [LodingAnimateView dissMissLoadingView];
        ...
        if ([[responseOrignal objectForKey:@"code"] intValue] == 200) {
            ...
            _arrData = [NSArray arrayWithArray:[BaolengZishuModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
            [self reloadData];
