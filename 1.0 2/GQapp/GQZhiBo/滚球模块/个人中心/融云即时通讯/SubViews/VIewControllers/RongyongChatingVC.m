...
@interface RongyongChatingVC ()

@end

@implementation RongyongChatingVC
...
- (void)onSelectedTableRow:(RCConversationModelType)conversationModelType
    ...
    if (model.conversationType == ConversationType_DISCUSSION || model.conversationType == ConversationType_GROUP) {
        RongYongGroupChatingVC *chatVC = [[RongYongGroupChatingVC alloc] init];
        [APPDELEGATE.customTabbar pushToViewController:chatVC animated:YES];
        
        
    }else{
        RongyunChatVC *conversationVC = [[RongyunChatVC alloc]init];
