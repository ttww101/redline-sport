...
- (void)setModel:(RecommandListModel *)model{
    ...
    if (self.type == 1 || self.type == 5) {//专家榜和盈利榜
        ...
        self.labSession.attributedText = [Methods withContent:self.labSession.text contentColor:color33 WithColorText:model.realnums textColor:color66];
    }else{
        ...
        if (self.type == 4 || self.type == 3) {
            ...
            self.labWinRate.attributedText = [Methods withContent:self.labWinRate.text WithColorText:@"%" textColor:color33 strFont:font14];
        }
    }
