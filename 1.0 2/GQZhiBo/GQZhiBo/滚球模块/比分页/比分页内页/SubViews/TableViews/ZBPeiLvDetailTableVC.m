...
@interface ZBPeiLvDetailTableVC ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) UITableView                          *tableView;
@property (nonatomic, strong) NSMutableArray                        *peiLvDetailArr;
@property (nonatomic, strong) NSMutableArray                        *peiLvAfterArr;

@end
@implementation ZBPeiLvDetailTableVC
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        [_tableView registerClass:[ZBPeiLvDetailCell class] forCellReuseIdentifier:peilvDetailCellID];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZBPeiLvDetailCell *peilvCell = [tableView cellForRowAtIndexPath:indexPath];
    if (!peilvCell) {
        peilvCell = [[ZBPeiLvDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:peilvDetailCellID];
    }
    ...
    if (self.PeiLvCDetailType == isDetailBeforeTwo) {
        ...
        if (_peiLvDetailArr.count > 0) {
            ZBPeiLvDetailModel *nowModel = _peiLvDetailArr[indexPath.row];
           
            if (_peiLvDetailArr.count > 1) {
                
                if (indexPath.row!= _peiLvDetailArr.count-1) {
                    
                    ZBPeiLvDetailModel *nextModel = _peiLvDetailArr[indexPath.row +1];
        ...
        if (_peiLvAfterArr.count > 0) {
            ...
            ZBPeiLvDetailModel *nowModel = _peiLvAfterArr[indexPath.row];
            
            if (_peiLvAfterArr.count > 1) {
                
                if (indexPath.row!= _peiLvAfterArr.count-1) {
                    
                    ZBPeiLvDetailModel *nextModel = _peiLvAfterArr[indexPath.row +1];
...
- (void)loadPeiLvDataWithScheid:(NSInteger)scheid OddsType:(oddsType)oddsType isHalf:(isHalfType)isHalf {
   ...
   // [SVProgressHUD showImage:[UIImage imageNamed:@""] status:@"数据加载中..."];
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:lsURl Start:^(id requestOrignal) {
        ...
        _peiLvDetailArr = [[NSMutableArray alloc] initWithArray:[ZBPeiLvDetailModel arrayOfEntitiesFromArray:responseOrignal]];
        if (_peiLvDetailArr.count>0) {
            ZBPeiLvDetailModel*model=_peiLvDetailArr[_peiLvDetailArr.count-1];
            model.BlackType=1;
