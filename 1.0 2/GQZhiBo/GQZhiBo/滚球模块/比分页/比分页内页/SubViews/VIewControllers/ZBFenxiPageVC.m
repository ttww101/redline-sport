...
@property (nonatomic, strong) ZBNewTuijianHtml *webViewZhiShu;//指数
@property (nonatomic, strong) ZBRecommendedWKWeb *recommendWeb;
@property (nonatomic , strong) ZBAnalysisWebview *analysisWeb; // 分析


@property (nonatomic, strong) ZBZhiboTableView *webZhiBo;//直播的页面
//@property (nonatomic, strong) ZBNewZhiBoWebView *webZhiBo;//直播的页面
...
{
    if (!_headerView) {
        _headerView = [[ZBFenxiHeaderView alloc] initWithFrame:CGRectMake(0, 0, Width, [(self.model.weather) isEqualToString:@""]? 164 : 190)]; // 160
...
//        ZBDan_StringMatchsModel *remodel = [[ZBDan_StringMatchsModel alloc] init];
        ZBDan_StringMatchsModel *remodel = [ZBDan_StringMatchsModel entityFromDictionary:[NSDictionary dictionary]];
        remodel.sid = _model.mid;
        remodel.hometeam = _model.hometeam;
        remodel.guestteam = _model.guestteam;
...
//- (ZBNewZhiBoWebView *)webZhiBo{
//    if (!_webZhiBo) {
//        _webZhiBo = [[ZBNewZhiBoWebView alloc] initWithFrame:CGRectMake(Width*4,0, Width, _scrollMainView.height)];
