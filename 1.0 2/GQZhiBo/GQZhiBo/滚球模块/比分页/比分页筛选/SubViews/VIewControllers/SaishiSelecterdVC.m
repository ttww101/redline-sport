...
@interface SaishiSelecterdVC ()<ViewPagerDelegate,ViewPagerDataSource,TitleIndexViewDelegate,SelectedAllVCDelegate,SelectedJincaiVCDelegate,SelectedChupanVCDelegate,NavViewDelegate>
@property (nonatomic, strong) TitleIndexView *titleView;
@property (nonatomic, assign) NSInteger currentIndex;
@end

@implementation SaishiSelecterdVC
...
- (UIViewController *)viewPager:(ViewPagerController *)viewPager contentViewControllerForTabAtIndex:(NSUInteger)index
{
    ...
    switch (index) {
        case 0:
        {
            SelectedAllVC *SelectedV  = [[SelectedAllVC alloc] init];
        ...
        case 1:
        {
            SelectedJincaiVC *SelectedV  = [[SelectedJincaiVC alloc] init];
            SelectedV.type = _type;
        ...
        case 2:
        {
            SelectedChupanVC *SelectedV  = [[SelectedChupanVC alloc] init];
            SelectedV.type = _type;
...
- (void)confirmSelectedChupanWithData:(NSArray *)arrSaveData
{
    ...
    if (_type == typeSaishiSelecterdVCBifen) {
        ...
        for (int i = 0; i<_arrBifenData.count; i++) {
            ...
            JSbifenModel *jsmodel = [_arrBifenData objectAtIndex:i];
            ...
            for (int m = 0; m<jsmodel.data.count; m++) {
                ...
                for (int j = 0; j<arrSaveData.count; j++) {
                    ...
                    BIfenSelectedSaishiModel *modelSave = [arrSaveData objectAtIndex:j];
    ...
    }else if (_type == typeSaishiSelecterdVCTuijian)
    {
        ...
        for (int i = 0; i<_arrBifenData.count; i++) {
            LiveScoreModel *model = [_arrBifenData objectAtIndex:i];
            NSArray *arrletgoal = [model.letgoal componentsSeparatedByString:@","];
            NSArray *arrtotal = [model.total componentsSeparatedByString:@","];
            NSArray *arrstandard = [model.standard componentsSeparatedByString:@","];
