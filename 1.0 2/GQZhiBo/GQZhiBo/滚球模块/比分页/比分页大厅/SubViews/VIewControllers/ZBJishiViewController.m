...
@interface ZBJishiViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate,DCindexBtnDelegate,FoldSectionHeaderViewDelegate, GQWebViewDelegate>
...
@property (nonatomic, strong) ZBDCJishiBIifenView *jsbfView;
...
@implementation ZBJishiViewController
...
//    ZBJsbfValue *model = [[ZBJsbfValue alloc] init];
//    model.time = @"66";
//    model.league =@"赛事";
...
//        // 更新界面
//        [[ZBMethods getMainWindow] addSubview:self.jsbfView];
//        
...
//    for (ZBJSbifenModel *jsModel in _arrData) {
//        
//        if (jsModel.data.count>0) {
//            
//            ZBLiveScoreModel *live = [jsModel.data objectAtIndex:0];
...
//    }
    ZBHSJFoldHeaderView *headerView = [ZBHSJFoldHeaderView new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZBSaiTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellJishiViewController];
    if (!cell) {
...
- (void)seperateAlldateArrayByArray:(NSMutableArray *)arr
{
    ...
    for (int i = 0; i<arr.count; i++) {
        ...
        ZBJSbifenModel *jsmodel = [arr objectAtIndex:i];
        
        
        for (int j = 0; j<jsmodel.data.count; j++) {
            
            
            ZBLiveScoreModel *model = [jsmodel.data objectAtIndex:j];
...
- (MBProgressHUD *)hud
{
    if (!_hud) {
        ...
        _hud = [[MBProgressHUD alloc] initWithView:[ZBMethods getMainWindow]];
        ...
        [[ZBMethods getMainWindow] addSubview:_hud];
...
//{
//    if (!_Hud) {
//        _Hud = [[MBProgressHUD alloc] initWithWindow:[ZBMethods getMainWindow]];
//        _Hud.mode= MBProgressHUDModeCustomView;
//        _Hud.animationType = MBProgressHUDAnimationFade;
//        _Hud.margin = 0;
//        _hud.cornerRadius = 0;
...
- (ZBDCJishiBIifenView *)jsbfView
{
    if (!_jsbfView) {
        _jsbfView = [[ZBDCJishiBIifenView alloc] initWithFrame:CGRectMake(20, Height - 49- 30 - 66, Width - 40, 66)];
        ZBJsbfValue *value = [[ZBJsbfValue alloc] init];
        _jsbfView.model = value;
        _jsbfView.userInteractionEnabled = NO;
        //        _jsbfView.alpha = 0;
        //        _jsbfView.hidden = YES;
        //        [[ZBMethods getMainWindow] addSubview:_jsbfView];
    }
    return _jsbfView;
}
        ...
        for (int i = 0 ; i<self.arrData.count; i++) {
            ...
            for (int m = 0; m<jsmodel.data.count; m++) {
                ...
                for (int j= 0; j<arr.count; j++){
                    ZBLivingModel *livingModel = [arr objectAtIndex:j];
                    ...
                    if (scoreModel.mid == livingModel.sid) {
                        ...
                        if (![[[NSUserDefaults standardUserDefaults] objectForKey:@"_currentflag"] isEqualToString:@"0"]) {
                            if (scoreModel.neutrality) {
                                ZBLivingModel *change = [[ZBLivingModel alloc] init];
                        ...
                        {
                            //在7点到23点之间的开启提示
                            if ([[[ZBMethods getDateByDate:[NSDate date] withWeekType:weekTypeXingqi]objectAtIndex:3] intValue]>=7 && [[[ZBMethods getDateByDate:[NSDate date] withWeekType:weekTypeXingqi]objectAtIndex:3] intValue]<=23)
                            {
                                //先判断是不是只提示我关注的
...
//红牌提示
- (void)hongpaiShowHudLivingModel:(ZBLivingModel *)livingModel NowModel:(ZBLiveScoreModel *)scoreModel
{
...
//获取比赛的时间
- (NSString *)getLivetime:(ZBLiveScoreModel *)scoreModel
{
    ...
    switch (scoreModel.matchstate) {
        ...
        {
            //上半场
            NSString *timeCha =[ZBMethods intervalFromLastDate:scoreModel.matchtime2 toTheDate:time];
        ...
        {
            //下半场
            NSString *timeCha = [ZBMethods intervalFromLastDateAnd45:scoreModel.matchtime2 toTheDate:time];
        ...
        case 4:
        {
            return  [ZBMethods intervalFromLastDateAnd45:scoreModel.matchtime2 toTheDate:time];
...
//{
//    //                NSLog(@"%@",responseOrignal);
//    NSMutableArray *arrLoad =[[NSMutableArray alloc] initWithArray:[ZBLiveScoreModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"matchs"]]];
//    _arrSelectedSaishi = [[NSArray alloc] initWithArray:[ZBBIfenSelectedSaishiModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"allindex"]]];
//    _arrSelectedSaishiJingcai = [[NSArray alloc] initWithArray:[ZBBIfenSelectedSaishiModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"jcindex"]]];
//    _arrSelectedSaishiChupan = [[NSArray alloc] initWithArray:[ZBBIfenSelectedSaishiModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"oddsindex"]]];
//
...
- (void)loadRedBombActivity {
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:[ZBHttpString getCommenParemeter]];
    [[ZBDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_redBomb]  Start:^(id requestOrignal) {
