...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        SearchVCZhujiaCell *cell = [tableView dequeueReusableCellWithIdentifier:flag];
        if (!cell) {
            cell = [[[NSBundle mainBundle]loadNibNamed:@"SearchVCZhujiaCell" owner:nil options:nil] firstObject];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (!_searchBar.isFirstResponder) {
        ...
        if (indexPath.section == 0) {
            ...
            UserlistModel *user = [_arrZhunjia objectAtIndex:indexPath.row];
...
- (void)searchTuijianInfoWithName:(NSString *)nickName
{
    ...
    [[DCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parmater PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] Start:^(id requestOrignal) {
...
//        _searchBar.showsCancelButton = YES;
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _arrZhunjia = [NSArray arrayWithArray:[UserlistModel  arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
            [_searchBar resignFirstResponder];
            [self.tableView reloadData];
...
{
    NSMutableDictionary *parmater = [NSMutableDictionary dictionaryWithDictionary:[HttpString getCommenParemeter]];
    [[DCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parmater PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_speciallist_hot] Start:^(id requestOrignal) {
