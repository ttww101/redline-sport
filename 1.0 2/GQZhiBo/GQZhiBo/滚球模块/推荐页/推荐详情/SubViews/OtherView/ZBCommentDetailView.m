...
@end
@implementation ZBCommentDetailView
...
*/
- (void)setModel:(ZBCommentChildModel *)model
{
    _model = model;
...
//    _labLikeCount.text = [NSString stringWithFormat:@"%ld",_model.likeCount];
    ...
    [_btnUser setAttributedTitle:[ZBMethods withContent:_btnUser.currentTitle WithContColor:color4C8DE5 WithContentFont:font14 WithText:@"回复" WithTextColor:color33 WithTextFont:font14] forState:UIControlStateNormal];

    
//    _labTime.text = [ZBMethods timeToNowWith:_model.createtime];
    
    
//    _labContent.text = _model.content;
    [_labContent setAttributedText:[ZBMethods setTextStyleWithString:_model.content WithLineSpace:3 WithHeaderIndent:0]];
    if (!_addAutoLayout) {
        [self adAutoLayout];
        _addAutoLayout = YES;
    }
...
//        _btnUser.backgroundColor = redcolor;
