...
#import "ZBCommentDetailView.h"


@interface ZBTuijianDetailCommentCell()<CommentDetailViewDelegate>
{
...
@implementation ZBTuijianDetailCommentCell
...
- (void)setModel:(ZBCommentModel *)model
{
    _model = model;
...
//    _btnAuthorPic.backgroundColor= redcolor;
    [_btnAuthor setText:_model.nickname];
    _labCreatTime.text = [ZBMethods timeToNowWith:_model.createtime];
    //    _model.content = [_model.content stringByReplacingOccurrencesOfString:@"---" withString:@"\n"];
    _labComentContent.text = _model.content;
    [_labComentContent setAttributedText:[ZBMethods setTextStyleWithString:_model.content WithLineSpace:3 WithHeaderIndent:0]];
    ...
    if (_model.child.count == 0) {
        ...
        for (int i = 0; i<count; i++) {
            ...
            ZBCommentChildModel *child = [_model.child objectAtIndex:i];
            
            
            CGFloat labchildhight = 25 +  [ZBMethods getTextHeightStationWidth:child.content anWidthTxtt:ViewBottomWidth - 5  anfont:13 andLineSpace:3 andHeaderIndent:0];
            
            ZBCommentDetailView *childView = [[ZBCommentDetailView alloc] initWithFrame:CGRectMake(0, viewbottomHight - 15, ViewBottomWidth, labchildhight)];
...
//    NSArray *constraints = _ViewBottom.constraints;
//    for (NSLayoutConstraint *constraint in constraints) {
//        
//        if (constraint.firstAttribute == NSLayoutAttributeHeight) {
//            constraint.constant = viewbottomHight;
//            break;
//        }
//        
//    }
...
- (void)showBtn
{
    if (self.viewHud.hidden) {
        for (ZBCommentDetailView *child in _arrCells) {
            [child hideHudView];
        }
...
- (void)touchChildCommentViewWithIdid:(NSInteger)Idid
{
    ...
    for (int i = 0; i<_arrCells.count; i++) {
        ZBCommentChildModel *child = [_model.child objectAtIndex:i];
        if (child.Idid != Idid) {
            ZBCommentDetailView *childView = [_arrCells objectAtIndex:i];
