...
@interface TuijianDetailCommentCell()<CommentDetailViewDelegate>
{
...
@implementation TuijianDetailCommentCell
...
//    _btnAuthorPic.backgroundColor= redcolor;
    ...
    _labCreatTime.text = [Methods timeToNowWith:_model.createtime];
    //    _model.content = [_model.content stringByReplacingOccurrencesOfString:@"---" withString:@"\n"];
    _labComentContent.text = _model.content;
    [_labComentContent setAttributedText:[Methods setTextStyleWithString:_model.content WithLineSpace:3 WithHeaderIndent:0]];
    ...
    if (_model.child.count == 0) {
        ...
        for (int i = 0; i<count; i++) {
            ...
            CommentChildModel *child = [_model.child objectAtIndex:i];
            
            
            CGFloat labchildhight = 25 +  [Methods getTextHeightStationWidth:child.content anWidthTxtt:ViewBottomWidth - 5  anfont:13 andLineSpace:3 andHeaderIndent:0];
            
            CommentDetailView *childView = [[CommentDetailView alloc] initWithFrame:CGRectMake(0, viewbottomHight - 15, ViewBottomWidth, labchildhight)];
...
- (void)showBtn
{
    if (self.viewHud.hidden) {
        for (CommentDetailView *child in _arrCells) {
            [child hideHudView];
        }
...
- (void)touchChildCommentViewWithIdid:(NSInteger)Idid
{
    ...
    for (int i = 0; i<_arrCells.count; i++) {
        CommentChildModel *child = [_model.child objectAtIndex:i];
        if (child.Idid != Idid) {
            CommentDetailView *childView = [_arrCells objectAtIndex:i];
