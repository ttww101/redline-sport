...
@interface ZBJiaoYiNewTabelView()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
//0 全部  1 热门  2竞彩  3北单
...
@implementation ZBJiaoYiNewTabelView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZBJiaoYiNewCell *cell = [tableView dequeueReusableCellWithIdentifier:acell];
    if (!cell) {
        cell = [[ZBJiaoYiNewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:acell];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZBJiaoYiModel *model = _dataArr[indexPath.row];
    if (!_isToFenxi == YES) {
        _isToFenxi = YES;
    }else{
        return;
...
- (void)getDataJiaoyi
{
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_JiaoYiList] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] intValue] == 200) {
            ...
            self.dataArr = [NSArray arrayWithArray:[ZBJiaoYiModel arrayOfEntitiesFromArray:responseOrignal[@"data"]]] ;
