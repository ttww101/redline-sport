...
@interface ZBPanwangZhishuTable()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@end
@implementation ZBPanwangZhishuTable
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[ZBPanwangCell class] forCellReuseIdentifier:cellPanwangCell];
...
{
    ZBPanwangCell *cell = [tableView dequeueReusableCellWithIdentifier:cellPanwangCell];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    if (!cell) {
        cell = [[ZBPanwangCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellPanwangCell];
    }
    cell.rankNum = indexPath.row + 1;
    ZBPanWangModel *model = self.arrData[indexPath.row];
    cell.model = model;
...
- (void)loadDataPanwangWithType:(loadDataType)loadType
{
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,@"/recordodds"] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] intValue] == 200) {
            ...
            NSArray *arr = [ZBPanWangModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]];
            if (arr.count == 0) {
