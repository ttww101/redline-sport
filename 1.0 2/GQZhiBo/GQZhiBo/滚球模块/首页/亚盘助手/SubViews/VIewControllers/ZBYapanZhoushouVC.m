...
#import "ZBPlycSelectedView.h"
#import "ZBYpasCell.h"
#import "ZBYpasTwoCell.h"
#define  cellYpasCell @"cellYpasCell"
#define  cellYpasTwoCell @"cellYpasTwoCell"
@interface ZBYapanZhoushouVC ()<TitleIndexViewDelegate,PlycSelectedViewdelegate,UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) ZBTitleIndexView *titleView;
@property (nonatomic, strong) ZBPlycSelectedView *selectedView;
...
@end
@implementation ZBYapanZhoushouVC
...
- (void)viewDidLoad {
    ...
    _titleView = [[ZBTitleIndexView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar, Width, 48)];
    ...
    _selectedView = [[ZBPlycSelectedView alloc] initWithFrame:CGRectMake(0, 0, Width, Height)];
...
- (void)loadPeilvYichangVCDataWithType:(loadDataType)type
{
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,_typeOdd == 0? url_letgoalAbnormalindex :url_totalAbnormalindex] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] integerValue] == 200) {
            ...
            NSArray *arr = [NSArray arrayWithArray:[ZBPlycModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
...
{
    if (!_tableView) {
        _tableView = [[ZBBasicTableView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar + 48 + 10, Width, Height - APPDELEGATE.customTabbar.height_myNavigationBar - 48 - 10) style:UITableViewStylePlain];
        _tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        _tableView.showsVerticalScrollIndicator = NO;
        [_tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"cell"];
        [_tableView registerClass:[ZBYpasCell class] forCellReuseIdentifier:cellYpasCell];
        [_tableView registerClass:[ZBYpasTwoCell class] forCellReuseIdentifier:cellYpasTwoCell];
...
{
    if (_currentIndex == 0) {
        ZBPlycModel *mod = [_arrdata objectAtIndex:indexPath.row];
...
{
    if (_currentIndex == 0) {
        ZBYpasCell *cell = [tableView dequeueReusableCellWithIdentifier:cellYpasCell];
        if (!cell) {
            cell = [[ZBYpasCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellYpasCell];
        }
        ...
        ZBYpasTwoCell *cell = [tableView dequeueReusableCellWithIdentifier:cellYpasTwoCell];
        if (!cell) {
            cell = [[ZBYpasTwoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellYpasTwoCell];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.model = [_arrdata objectAtIndex:indexPath.row];
        return cell;
