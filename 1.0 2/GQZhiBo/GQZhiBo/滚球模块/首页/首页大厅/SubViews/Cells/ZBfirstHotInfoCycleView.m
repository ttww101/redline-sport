...
@interface ZBfirstHotInfoCycleView()<UICollectionViewDataSource, UICollectionViewDelegate>
...
@end
@implementation ZBfirstHotInfoCycleView
...
- (void)setupMainView
{
    ...
    [mainView registerClass:[ZBFirstQBView class] forCellWithReuseIdentifier:@"FirstQBViewcell"];
...
{
    ZBFirstQBView *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"FirstQBViewcell" forIndexPath:indexPath];
...
- (void)setUpData:(NSArray *)arr{
    ...
    //        ZBFirstQBView *cellInfo = _arrView[i];
    //        cellInfo.model = arr[i];
    //    }
    
    for (int i = 0; i < arr.count; i++) {
        ZBLivingModel *living = [arr objectAtIndex:i];
        for (int j = 0; j < _arrData.count; j ++)
        {
            ZBFirstPInfoListModel *model = _arrData[j];
