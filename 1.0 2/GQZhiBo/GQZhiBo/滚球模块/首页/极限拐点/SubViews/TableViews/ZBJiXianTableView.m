...
@interface ZBJiXianTableView()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@implementation ZBJiXianTableView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZBJiXianCell *cell = [tableView dequeueReusableCellWithIdentifier:acell];
    if (!cell) {
        cell = [[ZBJiXianCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:acell];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZBRecord_OneModel *model = self.arrData[indexPath.row];
    if (!_isToFenxi == YES) {
        _isToFenxi = YES;
    }else{
        return;
...
- (void)loadDataJixian
{
    ...
    //1：胜平负  2：亚盘  3：大小球
    [parameter setObject:@( _currentType+ 1) forKey:@"oddstype"];//segenement
...
//    全部 热门 竞彩 北单 
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_recordodds] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] intValue] == 200) {
            ...
            _arrData = [NSArray arrayWithArray:[ZBRecord_OneModel arrayOfEntitiesFromArray:responseOrignal[@"data"]]];
