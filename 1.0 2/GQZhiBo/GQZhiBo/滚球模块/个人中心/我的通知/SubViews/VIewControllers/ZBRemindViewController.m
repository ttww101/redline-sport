...
@interface ZBRemindViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) ZBBasicTableView *tableView;
@property (nonatomic, strong) NSArray *arrData;
@end

@implementation ZBRemindViewController
...
{
    if (!_tableView) {
        self.tableView = [[ZBBasicTableView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar, Width, Height - APPDELEGATE.customTabbar.height_myNavigationBar) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZBRemindModel *model = [_arrData objectAtIndex:indexPath.row];
    ...
    labtime.text = [ZBMethods timeToNowWith:model.time];
    ...
    if (!model.iread) {
        ...
        labTitle.attributedText = [ZBMethods withContent:labTitle.text WithColorText:@"发布新推荐" textColor:color66 strFont:font12];
...
-  (void)loadUreadRemind
{
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_noticedo] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _arrData = [[NSArray alloc] initWithArray:[ZBRemindModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
...
-  (void)updateNoticeWithNoticeId:(ZBRemindModel *)model
{
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:[ZBHttpString getCommenParemeter]];
