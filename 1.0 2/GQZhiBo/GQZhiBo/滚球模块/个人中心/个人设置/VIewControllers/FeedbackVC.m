...
@interface FeedbackVC ()<UITextViewDelegate,UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@property (nonatomic, strong) FeedBackHeaderView                *feedBackHeaderView;
...
//    addImageLab.backgroundColor = redcolor;
    ...
    if ([Methods login]) {
        _telPhoneTextField.text = [Methods getUserModel].showmobile;
...
//        _feedBackHeaderView = [[FeedBackHeaderView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar, Width, Height - APPDELEGATE.customTabbar.height_myNavigationBar)];
//    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FeedBackCell *feedBackCell = [tableView cellForRowAtIndexPath:indexPath];
    if (!feedBackCell) {
        feedBackCell = [[FeedBackCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"feedBackCell"];
    }
...
- (void)getComments {
    ...
    [[DCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_feedback] ArrayFile:nil Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            _feedArr = [[NSMutableArray alloc] initWithArray:[FeedBackModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
