...
//推荐下面的统计数据
@property (nonatomic, strong) ZBUserTongjiAllModel *tongjiModel;
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[ZBUserOfotherCellTwo class] forCellReuseIdentifier:cellUserViewController];
        [_tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"cell"];
        [_tableView registerClass:[ZBTuijianDatingCell class] forCellReuseIdentifier:cellUserViewControllerRecommand];
        [_tableView registerClass:[ZBUserTongjiCell class] forCellReuseIdentifier:cellUserTongji];
        [_tableView registerClass:[ZBUserTongjiGoodPlayCell class] forCellReuseIdentifier:cellUserTongjiGoodPlay];
...
- (void)didToTongjiVC
{
    ...
    ZBTongjiVC *tongji = [[ZBTongjiVC alloc] init];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0:
            {
                return [tableView fd_heightForCellWithIdentifier:cellUserViewController configuration:^(ZBUserOfotherCellTwo * cell) {
    ...
    return 135;
//    return [tableView fd_heightForCellWithIdentifier:cellUserViewControllerRecommand cacheByIndexPath:indexPath configuration:^(ZBTuijianDatingCell *cell) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        switch (indexPath.row) {
            case 0:
            {
                ZBUserOfotherCellTwo *cell = [tableView dequeueReusableCellWithIdentifier:cellUserViewController];
                if (!cell) {
                    cell = [[ZBUserOfotherCellTwo alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserViewController];
                }
            ...
            case 1:
            {
                ZBUserTongjiCell *cell = [tableView dequeueReusableCellWithIdentifier:cellUserTongji];
                if (!cell) {
                    cell = [[ZBUserTongjiCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserTongji];
                }
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
            ...
            case 2:
            {
                ZBUserTongjiGoodPlayCell *cell = [tableView dequeueReusableCellWithIdentifier:cellUserTongjiGoodPlay];
                if (!cell) {
                    cell = [[ZBUserTongjiGoodPlayCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserTongjiGoodPlay];
                }
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
        ...
        ZBTuijianDatingCell *cell = [tableView dequeueReusableCellWithIdentifier:cellUserViewControllerRecommand];
        if (!cell) {
            cell = [[ZBTuijianDatingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserViewControllerRecommand];
        }
...
- (void)getCommentUserInfo
{
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:URLString Start:^(id requestOrignal) {
...
- (void)lodaRecommandData
{
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:URLString Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _arrRecommand = [NSArray arrayWithArray:[ZBTuijiandatingModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]] ;
...
- (void)loadTongjiData
{
    ...
    [[ZBDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:URLString Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            _tongjiModel = [ZBUserTongjiAllModel entityFromDictionary:[responseOrignal objectForKey:@"data"]];
            [self.tableView reloadData];
...
- (ZBTopContentView *)topView {
    if (_topView == nil) {
        _topView = [[ZBTopContentView alloc]init];
