...
@interface ZBUsersCell()<UserTuiianViewDelegate,UIGestureRecognizerDelegate,UINavigationControllerDelegate>
...
- (void)setModel:(ZBUserModel *)model
{
    ...
    _labGZ.hidden = ![ZBMethods login];
    _labFS.hidden = ![ZBMethods login];
    _labGZNum.hidden = ![ZBMethods login];
    _labFSNum.hidden = ![ZBMethods login];
...
//    _labUserRemark2.text = @" 7连红 ";
    ...
    //        ZBUsermarkModel *usermark = [remark objectAtIndex:i];
    //
    //        if (usermark.type == 1) {
...
//{
//    if (!_viewTuijian) {
//        _viewTuijian = [[ZBUserTuiianView alloc] init];
