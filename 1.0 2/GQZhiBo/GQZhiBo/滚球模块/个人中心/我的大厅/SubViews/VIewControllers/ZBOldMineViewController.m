...
@interface ZBOldMineViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ZBOldMineViewController
...
- (void)rightBarButtonItem
{
    if ([ZBMethods login]) {
        ...
        ZBNoticePageVC *noticeVC = [[ZBNoticePageVC alloc] init];
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[ZBUsersCell class] forCellReuseIdentifier:cellMineViewControllerUserCell];
...
- (void)renzheng
{
    ZBRealNameCerVC *real = [[ZBRealNameCerVC alloc] init];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section) {
        ...
        {
//            if ([ZBMethods login]) {
                return [tableView fd_heightForCellWithIdentifier:cellMineViewControllerUserCell configuration:^(ZBUsersCell* cell) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
            ...
            ZBUsersCell *cell = [tableView dequeueReusableCellWithIdentifier:cellMineViewControllerUserCell];
            if (!cell) {
                cell = [[ZBUsersCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellMineViewControllerUserCell];
            }
...
//    if (indexPath.section == 1 && indexPath.row ==2 && ![ZBMethods login] ) {
//        [ZBMethods toLogin];
//        return;
//    }
//    
...
//                    安全中心
                    ...
                    ZBUserModel *model = [ZBMethods getUserModel];
                    ZBAnQuanCenterVC *anquanVc = [[ZBAnQuanCenterVC alloc] init];
...
//                    ZBFeedbackNewVC *feed = [[ZBFeedbackNewVC alloc] init];
//                    feed.hidesBottomBarWhenPushed = YES;
//                    [APPDELEGATE.customTabbar pushToViewController:feed animated:YES];
