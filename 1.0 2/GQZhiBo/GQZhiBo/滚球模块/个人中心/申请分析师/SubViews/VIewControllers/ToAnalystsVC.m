...
@interface ToAnalystsVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,UITextViewDelegate,SuccessfulViewDelegate,AnalystsEventFilterVCDelegate>
...
@property (nonatomic, strong)UserModel *Newmodel;
@property (nonatomic, strong)NSString *strLeagues;
@property (nonatomic, strong)UILabel *labReason;
@property (nonatomic, strong)UIButton *sureBtn;
@property (nonatomic, strong) PictureView *picView1;
@property (nonatomic, strong) PictureView *picView2;
@property (nonatomic, strong) PictureView *picView3;


@property (nonatomic, assign) BOOL keyBShow;
@end

@implementation ToAnalystsVC
...
- (void)setModel:(UserModel *)model{
    _model = model;
    //    _Newmodel = [Methods getUserModel];
...
- (void)sureBtnCilck{
    ...
    if (![Methods isNameValid:self.txtName.text]) {
    ...
    if (self.txtTelPhone.text.length!= 11 ) { //|| ![Methods isMobileNumber:self.txtTelPhone.text]
...
//    NSArray *arrImagePic = [NSArray arrayWithObjects:_picView1.imageSave,_picView2.imageSave,_picView3.imageSave, nil];
    ...
    [[DCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] ArrayFile:nil Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            [Methods updateUsetModel:self.model];
            [self.tableView reloadData];
        }else{
...
- (PictureView *)picView1
{
    if (!_picView1) {
        _picView1 = [[PictureView alloc] init];
        _picView1.detailText = @"手持身份证";
    }
    return _picView1;
}
- (PictureView *)picView2
{
    if (!_picView2) {
        _picView2 = [[PictureView alloc] init];
        _picView2.detailText = @"身份证正面";
    }
    return _picView2;
}
- (PictureView *)picView3
{
    if (!_picView3) {
        _picView3 = [[PictureView alloc] init];
