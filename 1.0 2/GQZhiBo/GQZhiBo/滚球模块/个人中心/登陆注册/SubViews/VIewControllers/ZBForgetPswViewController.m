...
//    _checkTextF.borderStyle = UITextBorderStyleRoundedRect;
    ...
    _getCheckBTn = [[ZBJKCountDownButton alloc] initWithFrame:CGRectMake(_checkTextF.frame.origin.x + _checkTextF.frame.size.width - 10, textHeight + 10, 80, textHeight - 20)];
...
- (void)getCheckCode:(ZBJKCountDownButton *)btn
{
    ...
    if (![ZBMethods isMobileNumber:self.telTextF.text]) {
...
- (void)resetPsw
{
    ...
    /*
     ...
     authCode，//验证码
     */
    NSMutableDictionary *dictPatameter = [NSMutableDictionary dictionaryWithDictionary:[ZBHttpString getCommenParemeter]];
    [dictPatameter setObject:@"3" forKey:@"flag"];
    [dictPatameter setObject:_telTextF.text forKey:@"mobile"];
    [dictPatameter setObject:[ZBMethods md5WithString:_pswTextF.text] forKey:@"password"];
    ...
    [[ZBDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:dictPatameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] ArrayFile:nil Start:^(id requestOrignal) {
        if (!_prograssHud) {
