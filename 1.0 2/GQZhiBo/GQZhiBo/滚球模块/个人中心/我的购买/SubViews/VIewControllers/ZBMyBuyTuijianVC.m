...
@interface ZBMyBuyTuijianVC ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
{
    return [tableView fd_heightForCellWithIdentifier:cellMyBuyTuijianVC cacheByIndexPath:indexPath configuration:^(ZBTuijianDatingCell *cell) {
        if (_arrdata.count>0) {
...
{
    ZBTuijianDatingCell *cell = [tableView dequeueReusableCellWithIdentifier:cellMyBuyTuijianVC];
    if (!cell) {
...
- (void)lodaRecommandDataType:(loadDataType)loadType
{
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_buy_listUser] Start:^(id requestOrignal) {
