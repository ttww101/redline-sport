...
@interface ZBUserTuijianTable()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) NSMutableArray *arrData;
@property (nonatomic ,assign) NSInteger currentLimitStart;

@end
@implementation ZBUserTuijianTable
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[ZBTuijianDatingCell class] forCellReuseIdentifier:cellUserTuijianTable];
...
{
    return 135;
//    return [tableView fd_heightForCellWithIdentifier:cellUserTuijianTable cacheByIndexPath:indexPath configuration:^(ZBTuijianDatingCell *cell) {
//        if (_arrData.count>0) {
...
{
    ZBTuijianDatingCell *cell = [tableView dequeueReusableCellWithIdentifier:cellUserTuijianTable];
    if (!cell) {
        cell = [[ZBTuijianDatingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserTuijianTable];
    }
    cell.type = typeTuijianCellUser;
    if (_arrData.count>0) {
        cell.model = [_arrData objectAtIndex:indexPath.row];
...
- (void)lodaDataWithLoadtype:(loadDataType)loadDateType
{
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_newrecommendlistUser] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            NSArray *arr =[NSArray arrayWithArray:[ZBTuijiandatingModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
