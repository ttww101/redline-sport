...
@interface ZBNewQingBaoCell()

@property (nonatomic, strong)UIView *BGView;

@property (nonatomic, strong)UILabel *labTime;//联赛名称加时间
@property (nonatomic, strong)UILabel *labType;//情报的状态
@property (nonatomic, strong)UILabel *labTitle;//情报的标题
@property (nonatomic, strong)UILabel *labContent;//情报文字

@property (nonatomic, strong)UILabel *labHome;//主队名称
@property (nonatomic, strong)UILabel *labVs;
@property (nonatomic, strong)UILabel *labGues;

@property (nonatomic, strong)UIImageView *homeImg;//主队logo
@property (nonatomic, strong)UIImageView *guesImg;//客队logo



@property (nonatomic, strong)UILabel *labLeg;//联赛名称
...
@property (nonatomic, assign)BOOL isAddLayout;//判断

@end


@implementation ZBNewQingBaoCell
...
//        _labType.backgroundColor = redcolor;
        _labType.text = @"状态";
...
- (UILabel *)labContent{
    if (!_labContent) {
        ...
        _labContent.attributedText = [ZBMethods setTextStyleWithString:_labContent.text WithLineSpace:5.5 WithHeaderIndent:Width - 30];
    }
    return _labContent;
}
...
- (UILabel *)labHome{
    if (!_labHome) {
        ...
        _labHome.text = @"沙佩科恩斯";
//        _labHome.textAlignment = NSTextAlignmentRight;
    }
    return _labHome;
}
- (UILabel *)labVs{
...
- (UILabel *)labGues{
    if (!_labGues) {
        ...
        _labGues.text = @"沙佩科恩斯";
//        _labGues.textAlignment = NSTextAlignmentRight;
...
- (void)setModel:(ZBQingBaoFPTwoModel *)model{
    _model = model;
    self.labTime.text = [NSString stringWithFormat:@"发布于 %@",[ZBMethods getDateByStyle:@"MM月dd日 HH:mm" withDate:[NSDate dateWithTimeIntervalSince1970:[model.createtime doubleValue]/1000]]];
//    NSString *str = @"";
//    
//    switch (model.choice) {
//        case 3:
...
//    self.labType.text = model.tag;
//    self.labType.backgroundColor = [ZBMethods getColor:model.color];
    self.labTitle.numberOfLines = 2;
    self.labTitle.text = [NSString stringWithFormat:@"%@",model.title];
    
    self.labTitle.attributedText = [ZBMethods setTextStyleWithString:self.labTitle.text WithLineSpace:5 WithHeaderIndent:Width - 30];
//    self.labTitle.backgroundColor = redcolor;
    
    self.labContent.text = [NSString stringWithFormat:@"       %@",model.content];
    self.labHome.text = model.hometeam;
//    self.labHome.backgroundColor = redcolor;
    ...
    self.labFabuTime.text = [NSString stringWithFormat:@"%@",[ZBMethods getDateByStyle:dateStyleFormatterMdHm withDate:[NSDate dateWithTimeIntervalSince1970:[model.mtime doubleValue]/1000]]];
    self.labLeg.text = model.league;
    self.labLeg.textColor = [ZBMethods getColor:model.leagueColor];
    
    
    self.labContent.attributedText = [ZBMethods setTextStyleWithString:self.labContent.text WithLineSpace:5.5 WithHeaderIndent:Width - 30];
//    self.labContent.backgroundColor = redcolor;
...
//        make.bottom.equalTo(self.BGView.mas_bottom).offset(-22.5).priorityHigh();
    }];
...
//        make.left.equalTo(self.BGView.mas_left).offset(15);
//        make.top.equalTo(self.labContent.mas_bottom).offset(12.5);
//        make.bottom.equalTo(self.BGView.mas_bottom).offset(-17.5);
//    }];
