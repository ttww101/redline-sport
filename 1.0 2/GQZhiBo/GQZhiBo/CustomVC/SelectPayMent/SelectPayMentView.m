...
@interface SelectPayMentView () <OptionViewDelegate>
...
@property (nonatomic , strong) SelectPayMentView *isaView;
...
+ (instancetype)showPaymentInfo:(id)information
    ...
    static SelectPayMentView *payView = nil;
    if (!payView) {
        payView = [[SelectPayMentView alloc]initWithFrame:windowView.bounds];
    }
...
- (void)configUI {
    ...
    for (NSInteger i = 0; i < _options.count; i ++) {
        OptionView *view = [[OptionView alloc]initWithFrame:CGRectMake(0, 98 + 25 + i * 44, Width - 50, 44) configDictionary:_options[i]];
        view.deleate = self;
