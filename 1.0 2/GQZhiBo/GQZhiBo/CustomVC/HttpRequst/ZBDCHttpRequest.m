...
@interface ZBDCHttpRequest ()
@property (nonatomic, strong) UIView *animateBasicView;

@end
@implementation ZBDCHttpRequest
{
}
...
{
    static ZBDCHttpRequest *dcHttpRequset = nil;
    static dispatch_once_t onceInitDCHttpRequest;
    dispatch_once(&onceInitDCHttpRequest, ^{

        dcHttpRequset = [[ZBDCHttpRequest alloc] init];
    });
    
    if ([ZBMethods login]) {
        //[[NSUserDefaults standardUserDefaults] objectForKey:@"token"]
        [_afnetManager.requestSerializer setValue:[ZBMethods getTokenModel].token forHTTPHeaderField:@"token"];
...
//
//        [[ZBMethods getMainWindow] addSubview:_animateBasicView];
//    }
