...
- (void)resumePuchase {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[ZBAppleIAPService sharedInstance]VerifyingLocalCredentialsWithBlock:^(NSString *message, NSError *error) {
            if (error) {
...
- (void)configPageStatistics {
    ...
    if (dic) {
        ...
        if (![lastVersion isEqualToString:currentVersion]) {
            [ZBAspectManager GQ_SavePageDic];
        }
    } else {
        [ZBAspectManager GQ_SavePageDic];
    }
...
- (void)loadConfig {
    ...
    [[ZBDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_ConfigjSon] Start:^(id requestOrignal) {
...
- (void)setUpMessageSound {
    ...
    //设置系统音效
    [ZBMessageSoundMgr sharedInstanceForSound];
    //设置系统震动
    [ZBMessageSoundMgr sharedInstanceForVibrate];
}
...
//                    [[ZBDependetNetMethods sharedInstance] uploadImageWithImageArr:imageArr completion:^(BOOL finished, NSArray *arrUrl) {
//                        
//                        if (finished) {
//                            
...
- (void)initRootViewCotroller
{
    ...
    [self loadFirstData];
    //第二天打开app的时候重新初始化根视图
//    NSString *currentLocalDay = [ZBMethods getDateByStyle:DateFormatter withDate:[NSDate date]];
...
//    APPDELEGATE.url_JISHUIDATA=@"http://mobile.gunqiu.com:8803";//及时更新数据-及时赔率的
    ...
    //上线的时候关闭--wt
    APPDELEGATE.url_Server = @"http://mobile.gunqiu.com:81/interface/v3.6/";
    APPDELEGATE.url_jsonHeader = @"http://mobiledev.gunqiu.com";
    APPDELEGATE.url_ip = @"http://mobiledev.gunqiu.com:81";
    APPDELEGATE.url_ServerWWW = @"http://pctest.gunqiu.com";
    APPDELEGATE.url_ServerAgreement = @"http://www.gunqiu.com";
...
- (void)applicationWillEnterForeground:(UIApplication *)application {
    ...
    if ([ZBMethods login]) {      // 用户登出，清除 OutOfRefreshTokenTime / refreshTokentime
...
//    [ZBCacheObject delete_Data_urlKey:@"jiaoyi"];
//    [ZBCacheObject delete_Data_urlKey:@"touzhu"];
//    [ZBCacheObject delete_Data_urlKey:@"jixian"];
}
...
//
//    
//    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:@"http://mobile.gunqiu.com/interface/version" Start:^(id requestOrignal) {
//        
//    } End:^(id responseOrignal) {
//        
//    } Success:^(id responseResult, id responseOrignal){
    ...
    [[ZBDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_startpage] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            if (!(dataDic.allKeys.count == 0)) {
                ...
                [[ZBMethods getMainWindow] addSubview:launchV];
            }
        }
    } Failure:^(NSError *error, NSString *errorDict, id responseOrignal) {
