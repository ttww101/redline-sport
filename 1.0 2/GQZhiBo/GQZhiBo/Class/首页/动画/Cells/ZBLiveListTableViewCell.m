...
+ (ZBLiveListTableViewCell *)cellForTableView:(UITableView *)tableView {
    ...
    if (cell == nil) {
        cell = [[ZBLiveListTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
- (void)refreshContentData:(ZBLiveListModel *)model {
    ...
    [self.homeTermIcon sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://ls.sportradar.com/ls/crest/small/%@.png",model.homeUid]] placeholderImage:[ZBMethods defaultPlaceHolderImage:@"default_home_icon"]];
