...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[ZBUMStatisticsMgr sharedInstance] viewStaticsBeginWithMarkStr:@"ZBTuijianDetailVC"];
...
- (void)appleBuyWithData {
    ...
    [[ZBDCHttpRequest shareInstance]sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_purchase_recommend] ArrayFile:nil Start:^(id requestOrignal) {
        ...
        if (dic) {
            ...
            amount = amount * 100;
            [[ZBAppleIAPService sharedInstance]purchase:@{@"product_id":productID, @"orderID":_orderId, @"amount":@(amount)} resultBlock:^(NSString *message, NSError *error) {
                [hud hide:YES];
                if (error) {
